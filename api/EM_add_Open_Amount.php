<?php
/**
 * Class WH_AddOpenAmount_Action_After_Submit
 ...
 */
class WH_AddOpenAmount_Action_After_Submit extends \ElementorPro\Modules\Forms\Classes\Action_Base {
    /**
     * Get Name
     *
    ...
    public function get_label() {
        return __( 'whaddopenamount', 'text-domain' );
    }
    ...
    public function run( $record, $ajax_handler ) {
        ...
        if ( empty( $settings['whaddopenamount_url'] ) ) {
            return;
        }
        ...
        if ( empty( $settings['whaddopenamount_email_field'] ) ) {
            return;
        }
        ...
        // which is required by WHMCS's API to subsribe a user
        if ( empty( $fields[ $settings['whaddopenamount_email_field'] ] ) ) {
            return;
        }

        // CONFIG # https://docs.whmcs.com/API_Authentication_Credentials
        $whmcsUrl = $settings['whaddopenamount_url']; // ENTER Your whmcs URL
        $apiIdentifier = $settings['whaddopenamount_apiIdentifier']; // ENTER Your apiIdentifier
        $apiSecret = $settings['whaddopenamount_apiSecret']; // ENTER Your apiSecret
		...
		$itemamount1 = isset($_REQUEST['form_fields']['itemamount1']) && !empty($_REQUEST['form_fields']['itemamount1']) ? sanitize_text_field($_REQUEST['form_fields']['itemamount1']) : 'Empty';
		$itemdescription1 = isset($_REQUEST['form_fields']['itemdescription1']) && !empty($_REQUEST['form_fields']['itemdescription1']) ? sanitize_text_field($_REQUEST['form_fields']['itemdescription1']) : 'Empty';
		//$product_group_id = 1;
		//$product_id = 9;
		...
		// Add new open amount billble item
...
'itemtaxed1' => '1',
    ...
    public function register_settings_section( $widget ) {
        $widget->start_controls_section(
            'section_whaddopenamount',
            [
                'label' => __( 'whaddopenamount', 'text-domain' ),
                'condition' => [
    ...
    public function on_export( $element ) {
        unset(
            $element['whaddopenamount_url'],
            $element['apiIdentifier'],
            $element['apiSecret'],
            $element['whaddopenamount_name_field'],
            $element['whaddopenamount_email_field']
        );
    }
}